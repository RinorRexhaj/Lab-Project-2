...
const GroceriesPage: React.FC = () => {
  ...
  const handleStoreClick = async (store: GroceryStore) => {
    try {
      ...
      const storeWithProducts = await groceryService.getGroceryStoreWithProducts(store.id);
    ...
    if (value.trim() !== "") {
      ...
      const suggestions = stores
        .filter(s => 
          ...
          (s.category && s.category.toLowerCase().includes(value.toLowerCase()))
        )
        .map(s => s.name)
        .slice(0, 5);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-8 pb-16 bg-gray-50 min-h-screen">
      ...
      <section className="text-center py-14 px-3 mb-10 bg-gradient-to-r from-emerald-500 to-emerald-700 text-white rounded-2xl shadow-lg max-w-7xl w-full mx-auto">
        ...
        <p className="text-lg md:text-base mt-4">Fresh groceries delivered to your doorstep</p>
      </section>
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold flex items-center">
          <FontAwesomeIcon icon={faShoppingBasket} className="mr-3 text-emerald-500" />
          Grocery Stores
        </h2>
        <button 
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm mb-6 md:flex md:space-x-4">
          ...
          <div className="mb-4 md:mb-0 md:flex-1">
            ...
            <div className="flex flex-wrap gap-2">
              {['All', 'Supermarket', 'Convenience', 'Organic', 'Specialty', 'International', 'Wholesale'].map(category => (
                <div 
                  key={category}
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="text-sm text-gray-500">
          {sortedAndFilteredStores.length} store{sortedAndFilteredStores.length !== 1 ? 's' : ''}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          <p className="text-lg text-red-600">Error loading grocery stores. Please try again.</p>
        </div>
      ) : sortedAndFilteredStores.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-lg text-gray-600">No grocery stores found matching your search criteria.</p>
        </div>
      ) : (
